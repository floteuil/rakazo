...
describe("Requirement R2: Upstream Sync Security Gate Workflow E2E", () => {
  ...
  describe("Tier 2: Boundary & Corner Cases (R2 Gate Logic & Security Invariants)", () => {
    it("2.1 Guarantees zero hardcoded credentials or plaintext tokens in workflow file", () => {
      const forbiddenTokens = [/ghp_[a-zA-Z0-9]{36}/, /github_pat_[a-zA-Z0-9_]{82}/, /sk-[a-zA-Z0-9]{32,}/];
    ...
    it("2.2 Validates idempotency of git remote addition (handles existing upstream remote)", () => {
      expect(workflowContent).toMatch(/git\s+remote\s+add\s+upstream.*\|\|\s*git\s+remote\s+set-url\s+upstream/);
    });
    ...
    it("2.6 Simulates upstream sync state machine: clean merge vs conflict vs test failure paths", () => {
      type SyncState = "CHECK_COMMITS" | "MERGE_ATTEMPT" | "TEST_GATE" | "PUSH_MAIN" | "ABORT_AND_PR";

      function evaluateSyncFlow(hasNewCommits: boolean, mergeConflict: boolean, testGatePassed: boolean): SyncState {
      ...
      expect(evaluateSyncFlow(false, false, true)).toBe("CHECK_COMMITS");

      // Scenario B: New commits + Clean merge + Tests pass -> Push to main
      expect(evaluateSyncFlow(true, false, true)).toBe("PUSH_MAIN");

      // Scenario C: New commits + Merge conflict -> Abort and open PR
      expect(evaluateSyncFlow(true, true, true)).toBe("ABORT_AND_PR");

      // Scenario D: New commits + Clean merge + Tests FAIL -> Abort and open PR (critical gate invariant)
      expect(evaluateSyncFlow(true, false, false)).toBe("ABORT_AND_PR");
    });
  });
});
