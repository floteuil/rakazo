...
export const appContract = {
  ...
  skills: {
    list: oc.input(ListSkillsInput.optional()).output(z.array(SkillSummarySchema)),
    get: oc.input(GetSkillInput).output(SkillSchema),
    create: oc.input(CreateSkillInput).output(SkillSchema),
    update: oc.input(UpdateSkillInput).output(SkillSchema),
    delete: oc.input(DeleteSkillInput).output(z.object({ ok: z.literal(true) })),
    uploadMarkdown: oc.input(UploadSkillMarkdownInput).output(SkillSchema),
    assignToBot: oc
      .input(AssignSkillsToBotInput)
      .output(z.object({ ok: z.literal(true), count: z.number() })),
    getBotSkills: oc.input(GetBotSkillsInput).output(z.array(SkillSchema)),
  },
  taughtSkills: {
