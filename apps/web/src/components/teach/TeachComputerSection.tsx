...
export function TeachComputerSection({
  ...
  return (
    <div className="mt-[30px]">
      <div className="mb-3 text-[14px] text-[#85858A]">Enseigner une tâche</div>
      {!teachAvailable ? (
        <div className="rounded-[11px] border border-[#232326] px-3 py-3 text-[13.5px] leading-[1.5] text-[#6C6C70]">
          {computer?.kind === "desktop"
            ? "L'apprentissage visuel requiert une sandbox graphique. Cet agent exécute directement les commandes de terminal et scripts."
            : "Ouvrez la vue ordinateur pour enseigner une tâche."}
        </div>
      ) : recording ? (
        ...
        <div className="rounded-[11px] border border-[#232326] bg-[#121214] px-3 py-3">
          <label htmlFor="teach-goal-input" className="text-[13px] text-[#85858A]">
            Quel résultat souhaitez-vous démontrer ?
          </label>
          <textarea
            id="teach-goal-input"
            ...
            placeholder="Ex: Exporter la liste des clients du CRM et l'enregistrer dans le dossier partagé"
          />
      ...
      {saved.length > 0 ? (
        <>
          <div className="mt-[22px] mb-3 text-[14px] text-[#85858A]">Compétences enregistrées</div>
          {saved.map((skill) => (
