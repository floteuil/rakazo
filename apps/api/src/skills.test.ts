...
export class SkillsApiService {
  ...
  async listSkills(actor: Actor, query?: { search?: string; tag?: string; limit?: number; offset?: number }) {
    const list = Array.from(this.skills.values()).filter((s) => s.workspaceId === actor.workspaceId);
    let filtered = list;
